...
static const char* fragmentshader_source ="#version 410 core\n\
        ...
        {\n\
            float x = length(pos);\n\
            float a = 1.f / (1.f - 0.80f);\n\
            float b = 1.f - a;\n\
            float res = a * x + b;\n\
            color = vec4(1.0f-res, res, 0.f, 1.0f);\n\
        }\n";
...
ApproxErr::ApproxErr(int width, int height) : OpenGLDemo(width, height), _activecamera(0), _camera(nullptr) {

    float phi = (1.f+float(glm::sqrt(5.f)))/2.f;
    float w = 0.525731f;
    float l = w * phi;
    ...
    float edge = w*2.f;
    std::cout << "Volume icosaedre : "<< (5.f*(1.f+phi)*edge*edge*edge)/6.f << '\n';
    std::cout << "Volume Sphere : "<< 4.f* 3.14f *dist*dist*dist / 3.f << '\n';
...
void ApproxErr::resize(int width, int height){
...
void ApproxErr::mouseclick(int button, float xpos, float ypos) {
    _button = button;
    _mousex = xpos;
    _mousey = ypos;
...
void ApproxErr::mousemove(float xpos, float ypos) {
    _camera->processmousemovement(_button, xpos, ypos, true);
}

void ApproxErr::keyboardmove(int key, double time) {
    _camera->processkeyboard(Camera_Movement(key), time);
}

bool ApproxErr::keyboard(unsigned char k) {
    switch(k) {
        case 'p':
