...
Diffuse_light::Diffuse_light(int width, int height) : OpenGLDemo(width, height), _activecamera(0), _camera(nullptr) {
...
void Diffuse_light::resize(int width, int height){
    OpenGLDemo::resize(width, height);
    _camera->setviewport(glm::vec4(0.f, 0.f, _width, _height));
    _projection = glm::perspective(_camera->zoom(), float(_width) / _height, 0.1f, 100.0f);
}

void Diffuse_light::draw() {
    OpenGLDemo::draw();
...
void Diffuse_light::mouseclick(int button, float xpos, float ypos) {
    _button = button;
    _mousex = xpos;
    _mousey = ypos;
...
void Diffuse_light::mousemove(float xpos, float ypos) {
    _camera->processmousemovement(_button, xpos, ypos, true);
}

void Diffuse_light::keyboardmove(int key, double time) {
    _camera->processkeyboard(Camera_Movement(key), time);
}

bool Diffuse_light::keyboard(unsigned char k) {
    switch(k) {
        case 'p':
